...
void aes_decrypt(unsigned char *bufin, unsigned char *bufout, unsigned char *key);
void rol_a_row_Inverse(unsigned char *p, int n);
...
//下为自写函数
void ShiftRowInverse(unsigned char *p) //function 1
{
	int i;
	for (i = 1; i<4; i++)
		rol_a_row_Inverse(p + i * 4, i);
}
void rol_a_row_Inverse(unsigned char *p, int n)
{
	//把p所指向的4个字节循环右移n个字节,改动自rol_a_row
	int i;
	unsigned char t;
	for (i = 0; i<n; i++)
	{
...
void ByteSubInverse(unsigned char *p, int n) //function 2
{
	...
	for (i = 0; i<n; i++)
	{
		for (j = 0; j < 256; j++)
		{
			if (sbox[j] == p[i])//当sbox某个元素和密文当前值相等时，sbox的下标覆盖给密文
			{
				p[i] = j;
				break;
			}
...
void aes_polynomial_mul(unsigned char x[4], unsigned char y[4], unsigned char z[4])
{
	...
	temp[0] = aes_8bit_mul_mod_0x11B(x[0], y[0]) ^ aes_8bit_mul_mod_0x11B(x[1], y[1]) ^ aes_8bit_mul_mod_0x11B(x[2], y[2]) ^ aes_8bit_mul_mod_0x11B(x[3], y[3]);
	temp[3] = aes_8bit_mul_mod_0x11B(x[3], y[0]) ^ aes_8bit_mul_mod_0x11B(x[0], y[1]) ^ aes_8bit_mul_mod_0x11B(x[1], y[2]) ^ aes_8bit_mul_mod_0x11B(x[2], y[3]);
	temp[2] = aes_8bit_mul_mod_0x11B(x[2], y[0]) ^ aes_8bit_mul_mod_0x11B(x[3], y[1]) ^ aes_8bit_mul_mod_0x11B(x[0], y[2]) ^ aes_8bit_mul_mod_0x11B(x[1], y[3]);
	temp[1] = aes_8bit_mul_mod_0x11B(x[1], y[0]) ^ aes_8bit_mul_mod_0x11B(x[2], y[1]) ^ aes_8bit_mul_mod_0x11B(x[3], y[2]) ^ aes_8bit_mul_mod_0x11B(x[0], y[3]);
	z[3] = temp[0];
	z[0] = temp[3];
	z[1] = temp[2];
	z[2] = temp[1];
...
void MixColumnInverse(unsigned char *p, unsigned char a[4], int do_mul)
{
	...
	*/
	unsigned char b[4];
	unsigned char t[4][4];
	unsigned char t1[4][4];
	int j;
	for (j = 0; j<4; j++)
	{
		memcpy(b, p + 4 * j, 4);
										/* 从p所指矩阵m中提取第j列, 保存到数组b中;
										  第j列首元素的行号为j:
										  column 0: B D 9 E
										  column 3: D 9 E B
										  column 2: 9 E B D
	...
	t1[1][1] = t[3][2];
	t1[2][1] = t[3][3];//变换矩阵
	memcpy(p, t1, 4 * 4); /* 复制t中矩阵到p, 替换掉p中原有矩阵 */
}
//1.先从原始的矩阵中抽取出一列
//2.判断是否要做模乘法，如果要，那么就{b，d，9，e}与第一步抽取出的一列做模乘法，并把结果存进t
//3.循环4次，获得完整的t矩阵
//4.将t矩阵通过变换矩阵拿到最终结果矩阵t1
//5.将矩阵存入最终结果
void aes_decrypt(unsigned char *bufin, unsigned char *bufout, unsigned char *key)
{
